...
export function getMethodAndUrl(
  line: string,
  ...
  if (arr.length < 3) {
    throw new HttpError(
      HttpStatus.INTERNAL_SERVER_ERROR,
      "method, path, version is missing.",
    );
  }
  if (!checkMethod(arr[0])) {
    throw new HttpError(HttpStatus.INTERNAL_SERVER_ERROR, "method is invalid.");
  }
